...
def lot5(request):
    ...
    if res_num < 21:
        ...
        user_obj_list = Person.objects.filter(isWin=0,mWin=0,cWin=1)
        user_list = []
...
def update5(request):
    res_num = Result.objects.filter(awardname='五等奖',isdel=0).count()
    if res_num < 21:
        obj_list = Person.objects.filter(isWin=0,mWin=0,cWin=1)
...
def update4(request):
    ...
    if res_num < 10:
        ...
        if res_num==0:
            ...
            win_list = random.sample(user_list, 4)
        else:
            win_list=random.sample(user_list,4)
        for win in win_list:
            uid = win.get('id')
...
def update2(request):
    res_num = Result.objects.filter(awardname='二等奖',isdel=0).count()
    obj_list = Person.objects.filter(isWin=0, mWin=2, cWin=1)
    if obj_list:
        pass
    else:
        obj_list = Person.objects.filter(isWin=0, mWin=0, cWin=1)
...
def update1(request):
    res_num = Result.objects.filter(awardname='一等奖',isdel=0).count()
    obj_list = Person.objects.filter(isWin=0, mWin=1, cWin=1)
    if obj_list:
        pass
    else:
        obj_list=Person.objects.filter(isWin=0, mWin=0, cWin=1)
...
def updatet(request):
        res_num = Result.objects.filter(awardname='特等奖',isdel=0).count()
        obj_list = Person.objects.filter(isWin=0, mWin=10, cWin=1)
        if obj_list:
            pass
        else:
            obj_list = Person.objects.filter(isWin=0, mWin=0, cWin=1)
...
def updatej2(request):
    obj_list = Person.objects.filter(isWin=0, mWin=100, cWin=1)
    if obj_list:
        pass
    else:
        obj_list = Person.objects.filter(isWin=0, mWin=0, cWin=1)
